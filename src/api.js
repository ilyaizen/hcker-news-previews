...
const DEFAULT_BASE = "https://hn-clone.is-ai-good-yet.com";
...
window.YAHNC.fetchImages = async function fetchImages(ids, settings) {
  ...
  for (let i = 0; i < need.length; i += MAX_IDS_PER_REQUEST) {
    ...
    try {
      ...
      images = data.images || {};
      window.YAHNC.apiOk = true;
    } catch (err) {
      console.warn("[YAHNC] image fetch failed:", err);
      window.YAHNC.apiOk = false;

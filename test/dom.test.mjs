...
const YAHNC = sandbox.window.YAHNC;

test("idFromHref extracts the HN item id", () => {
  assert.equal(
    YAHNC.idFromHref("https://news.ycombinator.com/item?id=12345"),
    12345
  );
  assert.equal(YAHNC.idFromHref("https://example.com/x"), null);
  assert.equal(YAHNC.idFromHref(""), null);
  assert.equal(YAHNC.idFromHref(null), null);
});
...
test("dedupeById keeps exactly one entry per story id (first wins)", () => {
  ...
  const out = YAHNC.dedupeById(candidates);
...
test("dedupeById skips null / id-less candidates", () => {
  const out = YAHNC.dedupeById([null, { id: null }, { id: 5, row: {} }]);
